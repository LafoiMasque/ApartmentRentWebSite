using ApartmentRent.Core.LuceneNet.Interface;
using ApartmentRent.Core.LuceneNet.Model;
using ApartmentRent.Core.LuceneNet.Utility;
...
namespace ApartmentRent.Core.LuceneNet.Service
{
	public class LuceneBulid<T> : ILuceneBulid<T> where T : class, new()
	{
		...
		private PerFieldAnalyzerWrapper CreateAnalyzerWrapper(IEnumerable<FieldDataModel> fieldModelList)
		{
			...
			foreach (var fieldModel in fieldModelList)
			{
				...
				{
					analyzerWrapper.AddAnalyzer(fieldModel.FieldName, new StandardAnalyzer(LuceneUtil.Version.LUCENE_30));
				}
			}
